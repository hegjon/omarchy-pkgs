...
int main(int argc, char *argv[]) {
  ...
  // For OpenCL on Intel GPUs, there's intel-compute-runtime
  cl_platform_id cpu;
  for (int i = 0; i < n_platforms; i++) {
    err = clGetDeviceIDs(platform_id[i], CL_DEVICE_TYPE_CPU, 1, &device_id,
                         &n_devs);
    if (err == CL_SUCCESS) {
      cpu = platform_id[i];
      break;
  ...
  properties[1] = (cl_context_properties)cpu;
  properties[2] = 0;
  ...
  cl_mem x =
      clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof *xin * n, NULL, &err);
  if (err != CL_SUCCESS) {
  ...
  cl_mem y =
      clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof *xin * n, NULL, &err);
  if (err != CL_SUCCESS) {
  ...
  err = clEnqueueWriteBuffer(command_queue, x, CL_TRUE, 0, sizeof *xin * n, xin,
                             0, NULL, NULL);
  if (err != CL_SUCCESS) {
  ...
  err = clEnqueueNDRangeKernel(command_queue, kernel, 1, NULL, &n, NULL, 0,
                               NULL, NULL);
  if (err != CL_SUCCESS) {
  ...
  err = clEnqueueReadBuffer(command_queue, y, CL_TRUE, 0, sizeof *yout * n,
                            yout, 0, NULL, NULL);
  if (err != CL_SUCCESS) {
